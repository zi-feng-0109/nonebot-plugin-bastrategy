...
# 撤回消息
che = on_keyword({'广告', '沙雕', '广告', '操你妈', '操你', 'CNM','CNm','CnM','Cnm','cNM','cNm','cnM'})
...
async def j(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if qq == zr:
        ...
        await che.finish(message=f'好的sensei，阿罗那已经为您开启全体禁言了')
    else:
...
async def g(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    try:
        if qq == zr:
            ...
            await che.finish(message=f'好的sensei，阿罗那已经为您关闭全体禁言了')
        else:
            await che.finish(message=f'你没有资格命令我！')
    except IndexError:
        k = 'error'
...
async def sj(bot: Bot, event: GroupMessageEvent, state: T_State):
    try:
        ...
        if qq2 == zr:
            ...
            await jy.finish(message=f'好的sensei，阿罗那已禁言\nQQ:{qq}\n时间：{sj1[0]}分钟')
        else:
...
async def sj(bot: Bot, event: GroupMessageEvent, state: T_State):
    try:
        ...
        if qq2 == zr:
            ...
            await jj.finish(message=f'好的sensei，阿罗那已解除禁言\nQQ:{qq}')
        else:
